...
let processInputLine (strippedSourceFileName:string) (fileLine:string):FlattenedTask option = 
  ...
  with |_->None 
let transformFileLines (strippedSourceFileName:string) (fileLines:string []):seq<FlattenedTask option> = 
  let ret:seq<FlattenedTask option> = 
    fileLines |> Seq.map(fun line->
      processInputLine strippedSourceFileName line
      )
  ret
let processInputFile (fileName:String):(string*seq<FlattenedTask>) option = 
